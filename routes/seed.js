/**
 * Recruiting Seeder Endpoint
 * POST /api/seed/recruiting-candidates
 */
...
router.post('/recruiting-candidates', async (req, res) => {
    try {
        ...
        for (const candidate of candidatesData) {
            try {
                await db.query(
                    ...
                    [candidate.name, candidate.email, candidate.phone, candidate.company, candidate.equipment, 
                     candidate.mc_number, candidate.authority_age, candidate.safety_rating, candidate.source_id, candidate.status]
                );
                addedCount++;
            } catch (e) {
                // Skip duplicates
        ...
        res.json({ success: true, added: addedCount, total: candidatesData.length });
    } catch (error) {
